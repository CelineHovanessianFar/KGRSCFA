...
TMP_DIR = {
    ...
    'cell': os.path.join('.', 'CAFE', 'tmp', 'Cellphones_Accessories'),
    'clothing': os.path.join('.', 'CAFE', 'tmp', 'Clothing'),
    'cd': os.path.join('.', 'CAFE', 'tmp', 'CDs_Vinyl'),
}
...
def parse_args():
    ...
    # TODO
    parser.add_argument('--user_sample_size', type=int, default=120, help='Sample size of users for KG shrinkage.')
    ...
    # checkpoint name of the trained neural-symbolic model.
    args.symbolic_model = os.path.join(args.log_dir, f'symbolic_model_epoch{args.epochs}.ckpt')

    # filename of the paths inferred by the trained neural-symbolic model.

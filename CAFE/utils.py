...
def save_embed(dataset, embed):
    ...
    embed_dir = TMP_DIR[dataset]
    os.makedirs(embed_dir, exist_ok=True)

    # Save the embedding file
    embed_file = os.path.join(embed_dir, 'embed.pkl')
...
def save_labels(shrunk_labels, dataset, mode='train'):
    if mode == 'train':
        save_file = LABEL_FILE[dataset][0]
    elif mode == 'test':
        save_file = LABEL_FILE[dataset][1]
    else:
        raise Exception('mode should be one of {train, test}.')

    with gzip.open(save_file, 'wt') as f:
        for user_id, items in shrunk_labels.items():

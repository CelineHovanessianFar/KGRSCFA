...
class OnlinePathLoader:
    def __init__(self, dataset, batch_size, topk=20):
        ...
        # self.valid_users = list(self.kg.get(USER).keys())
...
class OnlinePathLoaderWithMPSplit:
    def __init__(self, dataset, batch_size, topk=20, mpsplit_ratio=0.7):
        # Load knowledge graph info.
        self.kg = load_kg(dataset)
        self.num_users = len(self.kg.get(USER))
        self.num_products = len(self.kg.get(PRODUCT))
...
def test_online_path_loader(args):
    set_random_seed(123)
    dataloader = OnlinePathLoader(args.dataset, args.batch_size)
    steps = 0
...
def test_online_path_loader_with_mpsplit(args):
    set_random_seed(123)
    dataloader = OnlinePathLoaderWithMPSplit(args.dataset, args.batch_size, mpsplit_ratio=0.7)
    steps = 0
...
def test_kgmask(args):
    ...
    kgmask = KGMask(kg)
    pids = kgmask.get_mask(USER, [1, 2, 3], PURCHASE)
    print(pids.shape)
    pids = kgmask.get_mask(USER, 1, PURCHASE)
    print(pids.shape)
    pids = kgmask.get_et(USER, 1, PURCHASE)
    print(pids)
